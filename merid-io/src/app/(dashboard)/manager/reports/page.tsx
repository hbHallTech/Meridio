...
export default function ManagerReportsPage() {
  ...
  const clearFilters = () => { setTeamId(""); setTypeId(""); setStatus(""); setFrom(""); setTo(""); };

  const hasFilters = teamId || typeId || status || from || to;

  const handleExportCSV = () => {
    if (!data) return;
    const header = ["Employee", "Email", "Team", "Type", "Start", "End", "Days", "Status"];
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">{lang === "en" ? "Team Reports" : "Rapports Equipe"}</h1>
          {data && (
                ...
                {data.items.map((item) => {
                  ...
                  return (
                    <tr key={item.id} className="hover:bg-gray-50/50">
                      ...
                      <td className="whitespace-nowrap px-6 py-4 text-gray-600">{item.user.team?.name ?? "—"}</td>
                      <td className="whitespace-nowrap px-6 py-4">
