...
export async function POST(
  ...
  if (!["PENDING_MANAGER", "PENDING_HR"].includes(leaveRequest.status)) {
    return NextResponse.json(
      { error: "Cette demande ne peut pas être refusée dans son état actuel" },
      { status: 400 }
    );
  }
  ...
  await prisma.notification.create({
    data: {
      ...
      body_fr: `Votre demande de congé a été refusée. Motif : ${comment.trim()}`,
      body_en: `Your leave request has been refused. Reason: ${comment.trim()}`,
  ...
  try {
    ...
    console.error("[EMAIL ERROR] Failed to send refusal email:", e);
  }
  ...
  await prisma.auditLog.create({
    data: {
      ...
      entityId: id,
      newValue: { status: "REFUSED", comment: comment.trim(), stepId: pendingStep.id },
    },
  });
