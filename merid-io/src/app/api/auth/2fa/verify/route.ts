...
export async function POST(request: NextRequest) {
  try {
    // BotID check removed — route requires auth() and has its own
    // brute-force protection (MAX_2FA_ATTEMPTS).
    ...
    console.error("[2fa/verify] Unhandled error:", err instanceof Error ? err.message : err);
    return NextResponse.json(
