...
// ─── GET: fetch personal info for authenticated user ───
...
// ─── PATCH: update personal info (self-service) ───
...
export async function PATCH(request: NextRequest) {
  ...
  try {
    ...
    // Audit log — sanitize sensitive personal data
    await prisma.auditLog.create({
      data: {
        ...
        newValue: { fieldsUpdated: Object.keys(updateData) },
      },
    });

    return NextResponse.json(updated);
  } catch (error) {
    console.error("PATCH /api/users/me/personal error:", error);
    return NextResponse.json(
