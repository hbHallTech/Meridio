  ...
  await sendEmail({
    ...
    html: emailWrapper(`
      ...
      <table style="width: 100%; border-collapse: collapse; margin: 16px 0; background: #f8fafc; border-radius: 8px;">
        <tr><td style="padding: 12px 16px; color: #6b7280;">Email de connexion</td><td style="padding: 12px 16px; font-weight: 600;">${escapeHtml(to)}</td></tr>
...
export async function sendSignupRejectionEmail(
  to: string,
  firstName: string,
  companyName: string,
  notes?: string | null
) {
  const contactUrl = `${process.env.NEXT_PUBLIC_APP_URL || "http://localhost:3000"}/#contact`;
