...
const ROUTE_ROLE_MAP: { prefix: string; roles: UserRole[] }[] = [
  ...
  // HR can access user management, teams (read) and offices (read) pages and APIs
  { prefix: "/admin/users", roles: ["HR", "ADMIN", "SUPER_ADMIN"] },
  { prefix: "/api/admin/users", roles: ["HR", "ADMIN", "SUPER_ADMIN"] },
  { prefix: "/api/admin/teams", roles: ["HR", "ADMIN", "SUPER_ADMIN"] },
  { prefix: "/api/admin/offices", roles: ["HR", "ADMIN", "SUPER_ADMIN"] },
  ...
  callbacks: {
    ...
    authorized({ auth, request: { nextUrl } }) {
      try {
        ...
        console.error("[middleware/authorized] CRASH:", err instanceof Error ? err.message : err);
        // Never crash silently — allow the request through rather than blocking everything
        return true;
      }
    },
  },
